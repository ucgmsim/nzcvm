...
# Layers have a spatial domain (``Layer.geometry``), but as of writing no
...
def test_clamp_vs_untouched_leaves_vp_rho_alone() -> None:
    ...
    # vs 3500 already satisfies the bound. The clamp snaps nothing.
...
# comparison reversed. The test parametrises over ``(config field, comparison)``
# rather than duplicating it.  Note @pytest.mark.parametrize must sit *outside*
# @given.
@pytest.mark.parametrize(
    "field, compare",
    [
...
def test_adhoc_functional_layer_deserialises(isolated_layer_registry: None) -> None:
    ...
    # Config must have the right type tag
...
def test_offshore_suppressed_under_a_basin_that_reaches_the_surface() -> None:
    ...
    # i == 1 is basin-free and offshore. The profile applies there.
