...
class CoastlineLayer(Layer[CoastlineConfig], config_cls=CoastlineConfig):
    ...
    def __getstate__(self):
        # The Rust index has no Python representation, so hand dask a registry
        # key and keep the object in this process, as Surface and ModelTree do.
        state = self.__dict__.copy()
        state["coastline"] = registry.pickle_pass(self.coastline)
        return state

    def __setstate__(self, state):
        self.__dict__.update(state)
        self.coastline = registry.REGISTRY[state["coastline"]]
